...
export default function Navbar() {
  ...
  return (
    <div className='bg-gray-700 flex items-center px-5 py-2 z-50 justify-between w-full fixed top-0 left-0 right-0'>
      ...
      <div className='h-full m-0 md:m-auto justify-self-center flex-2'>
        <button
          className='w-12 inline-block md:hidden mr-3 float-right'
          onClick={() => {
            setSideHidden(!sideHidden);
          }}>
          <div className=''>
            <div
              className={`transition duration-500 w-full bg-white rounded-lg h-1 mb-1 ${
                !sideHidden && 'rotate-45  translate-y-2'
              }`}></div>
            <div
              className={`transition duration-500 w-full bg-white rounded-lg h-1 mb-1 ${
            ...
            <div
              className={`transition duration-500 w-full bg-white rounded-lg h-1 ${
                !sideHidden && '-rotate-45  -translate-y-2'
              }`}></div>
          </div>
        ...
        {JSON.parse(localStorage.getItem('user')) ? (
          <button
            ...
            <div
              ...
              <Link
                to='/login'
                className='hover:bg-slate-400 w-full transition py-2'>
          ...
          <Link
            to='/login'
            className='text-gray-300 block w-max ml-auto pl-4 mr-3 z-30 hover:underline'>
            Login / Sign Up
          </Link>
        )}
      </ol>
