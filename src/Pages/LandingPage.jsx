...
function LandingPage() {
  ...
  const formInputReducer = (state, action) => {
    if (action.type === 'changeToUserType') {
      ...
      if (state.isSignUp) {
        hero.current.classList.remove(
          'md:-translate-x-[100%]',
          'lg:-translate-x-[233.3%]'
        );
        hero.current.classList.add('md:translate-x-[0%]');
        landingSignup.current.classList.remove(
          'md:-translate-x-[100%]',
          'lg:-translate-x-[233.3%]'
        );
    ...
    if (action.type === 'changeToBusinessType') {
      ...
      if (state.isSignUp) {
        hero.current.classList.add(
          'md:-translate-x-[100%]',
          'lg:-translate-x-[233.3%]'
        );
        hero.current.classList.remove('md:translate-x-[0%]');
        landingSignup.current.classList.add(
          'md:-translate-x-[100%]',
          'lg:-translate-x-[233.3%]'
        );
    ...
    if (action.type === 'changeSignUp') {
      if (!state.isSignUp && state.userType === 'business') {
        ...
        businessInfoSlider.current.classList.remove(
          'md:-translate-x-[100%]',
          'lg:-translate-x-[233.3%]'
        );
        businessInfoSlider.current.classList.remove('md:translate-x-[0%]');
  ...
  return (
    <section ref={section} className=''>
      <div
        ...
        <div
          ref={landingSignup}
          className='transition duration-300 overflow-hidden relative border landing__signup w-full bg-blue-900 block md:flex flex-col items-center justify- place-content-center pb-12 pt-36 md:pt-12 px-10 h-full text-center text-neutral-100 border-blue-900'>
          ...
          <form action='' className='flex flex-col items-center gap-4 w-full'>
            <div className='flex w-3/4 relative isolate bg-slate-400 rounded-[1.5rem] mb-4 cursor-pointer'>
              ...
              <div
                ref={userSelect}
                className='block my-auto transition w-1/2 px-2 py-3 z-10 rounded-[1.5rem] font-bold '
                onClick={() => dispatch({ type: 'changeToUserType' })}>
                Sign In
              </div>
              <div
                ref={businessSelect}
                className='transition w-1/2 px-2 py-3 z-10 rounded-[1.5rem] font-bold opacity-60'
                onClick={() => dispatch({ type: 'changeToBusinessType' })}>
                Sign Up
              </div>
              <div
                ref={indicator}
                className='absolute transition duration-300 inset-y-0 w-1/2 left-0 bg-blue-500 z-0 rounded-[1.5rem]'></div>
            </div>
            ...
            {!loggingIn ? (
              <button
                type='submit'
                className='btn-primary mt-4 w-9/12 mb-5 py-2 rounded bg-blue-500 hover:bg-blue-600 transition text-neutral-100 font-semibold'
        ...
        <div
          ref={businessInfoSlider}
          className='transition duration-300 text-white mt-0 pb-9 md:pb-0 h-auto w-full bg-blue-900 flex flex-col items-center justify-center gap-5'>
          <div className='business-logo flex flex-col items-center justify-center w-full pb-5'>
            ...
            <input
              className='block mx-auto text-center w-full md:w-auto'
              type='file'
              name='Image Upload'
              id=''
              onChange={async (e) => {
          ...
          <div className='general-information w-full h-auto'>
            ...
            <label htmlFor='business-description' className='block text-center'>
              Description
            </label>
            <textarea
              className='block mt-4 py-2 px-4 rounded text-neutral-900 mx-auto w-[90%] md:w-[60%]'
              name='description'
