...
function Profile() {
  ...
  useEffect(() => {
    if (localStorage.getItem('user') && state) {
      ...
      sendRequest('product/showUser', 'POST', {
        body: {
          userID:
            userID.length < 10 ? JSON.parse(getLocal('user'))._id : userID,
        },
      }).then((products) => {
  ...
  return (
    !loading &&
    userData !== null && (
      <div className='w-full flex flex-col gap-12 py-16 px-6 bg-[#404959] text-[#eee] min-h-screen'>
        <div className='flex gap-12 items-center flex-col xl:flex-row'>
          ...
          <div className='xl:border-l w-full ml-0 xl:mx-4'>
            <RowDisplay title={'Favorited Items'}>
              {userFavorites.length > 0 && userFavorites[0] != 'HIDDEN' ? (
                userFavorites.map((data, idx) => {
                  return (
                    <ProductCard
                      type={'favorite'}
                      key={idx}
                      image={basePath + data.images[0]}
                      title={data.name}
                      price={data.price}
                      location={data.Location}
                      id={data._id}
                      link={`/productDetail?id=${data._id}`}
                    />
                  );
                })
              ) : userFavorites[0] == 'HIDDEN' ? (
                <p className=''>This User's Favorited Items are Hidden</p>
              ) : (
                <p className=''>No Favorited Items</p>
              )}
            </RowDisplay>
          </div>
        </div>
                ...
                userProducts.length > 0 ? (
                  userProducts.map((data, idx) => {
                    ...
                    return (
                      <ProductCard
                        type={
                          search.length > 10 && search != userID
                            ? 'favorite'
                            : 'edit'
                        }
                        key={idx}
